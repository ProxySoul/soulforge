...
**Local provider overrides:** Ollama defaults to `localhost:11434` (override with `OLLAMA_HOST`). LM Studio defaults to `localhost:1234` (override with `LM_STUDIO_URL`, optional auth with `LM_API_TOKEN`).
