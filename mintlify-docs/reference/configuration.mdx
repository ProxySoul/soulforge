...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `toolTimeout` | `number` | `2` | Tool call timeout in minutes. Applies to shell, project, and agent tools. `0` disables timeout. Change via `/timeouts`. |
...
SoulForge loads enabled instruction files from both your project root and your home directory. For example, the `soulforge` source can come from `SOULFORGE.md` in the repo or `~/.soulforge/instructions.md`, and the `codex` source can come from `AGENTS.md` in the repo or `~/.agents/instructions.md`. When both scopes exist, global instructions are loaded too and take priority over project-local instructions.
