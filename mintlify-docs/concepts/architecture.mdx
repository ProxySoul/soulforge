...
Every turn, the prompt is assembled from: mode, project info, git context, the repo map, your memory, forbidden files, active skills. The repo map is personalized per-turn — files you just edited or referenced get boosted.

See [repo map](/concepts/repo-map) for ranking details, [compound tools](/concepts/compound-tools) for the one-call tools, [code intelligence](/concepts/intelligence) for the backend router.